...
for iDataIdx = 1:length(dataname)
    ...
    All_Results = struct('Lambda', {}, 'm_Ratio', {}, 'k_Neighbor', {}, 'MissingRatio', {}, 'NumFolds', {}, ...
                         'ACC', {}, 'NMI', {}, 'Purity', {}, 'AR', {}, 'RI', {}, ...
                         'MI', {}, 'HI', {}, 'Fscore', {}, 'Precision', {}, 'Recall', {}, ...
                         'ACC_STD', {}, 'NMI_STD', {}, 'Purity_STD', {}, 'AR_STD', {}, 'RI_STD', {}, ...
                         'MI_STD', {}, 'HI_STD', {}, 'Fscore_STD', {}, 'Precision_STD', {}, 'Recall_STD', {}, ...
                         'CoreTime', {}, 'TotalTime', {}, 'Time', {}, ...
                         'CoreTime_STD', {}, 'TotalTime_STD', {}, 'Time_STD', {});
    res_counter = 0;
    ...
    for iSplit = 1:nSplits
        ...
        nView = length(Xs);
        if numel(folds) < nfolds
            error('%s contains %d folds; %d folds are required.', datafile, numel(folds), nfolds);
        end
        ...
        fold_metrics = zeros(nfolds, total_combinations, 10); % my_eval_y returns 10 metrics
        fold_times = zeros(nfolds, total_combinations, 2); % [CoreTime, TotalTime]

        fprintf('>>> Running %s %s : %d folds x %d combinations sequentially...\n', ...
            cell2mat(dataname(iDataIdx)), cell2mat(numname(iSplit)), nfolds, total_combinations);

        for iFold = 1:nfolds
            missInd = folds{iFold};

            % --- 1. Physical Filtering (Done once per fold) ---
            ...
            for combo_idx = 1:total_combinations
                ...
                fprintf('   Fold %d/%d [%d/%d] Lambda=%.1f, m_ratio=%d, k=%d | ACC=%.4f | CoreT=%.2fs, TotalT=%.2fs\n', ...
                    iFold, nfolds, combo_idx, total_combinations, cur_lambda, cur_m_ratio, cur_k_neighbor, ...
                    result(1), core_time, total_time);

                fold_metrics(iFold, combo_idx, :) = reshape(result, 1, 1, 10);
                fold_times(iFold, combo_idx, :) = reshape([core_time, total_time], 1, 1, 2);
            end
        end

        all_metrics = reshape(mean(fold_metrics, 1), total_combinations, 10);
        all_metrics_std = reshape(std(fold_metrics, 0, 1), total_combinations, 10);
        all_times = reshape(mean(fold_times, 1), total_combinations, 2);
        all_times_std = reshape(std(fold_times, 0, 1), total_combinations, 2);
        ...
        for combo_idx = 1:total_combinations
            ...
            result_std = all_metrics_std(combo_idx, :);
            ...
            All_Results(res_counter).NumFolds = nfolds;
            ...
            All_Results(res_counter).ACC_STD = result_std(1);
            All_Results(res_counter).NMI_STD = result_std(2);
            All_Results(res_counter).Purity_STD = result_std(3);
            All_Results(res_counter).AR_STD = result_std(4);
            All_Results(res_counter).RI_STD = result_std(5);
            All_Results(res_counter).MI_STD = result_std(6);
            All_Results(res_counter).HI_STD = result_std(7);
            All_Results(res_counter).Fscore_STD = result_std(8);
            All_Results(res_counter).Precision_STD = result_std(9);
            All_Results(res_counter).Recall_STD = result_std(10);
            ...
            All_Results(res_counter).CoreTime_STD = all_times_std(combo_idx, 1);
            All_Results(res_counter).TotalTime_STD = all_times_std(combo_idx, 2);
            All_Results(res_counter).Time_STD = all_times_std(combo_idx, 2);
        end
